    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="p-6 border-b border-gray-100">
        <div class="flex items-center justify-between">
          ...
          <button onclick="openModal('add')" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-lg font-semibold flex items-center space-x-2 transition">
            ...
            @forelse($printers as $printer)
              <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4">
                  ...
                  @else
                    <button onclick="setDefault({{ $printer->id }})" class="text-xs text-gray-500 hover:text-amber-600 transition">
                      Set as default
                    </button>
                  @endif
                </td>
                <td class="px-6 py-4 text-right">
                  <div class="flex items-center justify-end space-x-2">
                    <button onclick="testPrint({{ $printer->id }})" class="p-2 text-green-600 hover:bg-green-50 rounded-lg transition" title="Test Print">
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                      </svg>
                    </button>
                    <button onclick='editPrinter(@json($printer))' class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Edit">
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      </svg>
                    </button>
                    <form action="{{ route('admin.printers.destroy', $printer) }}" method="POST" class="inline" onsubmit="return confirm('Hapus printer {{ $printer->name }}?')">
                      @csrf
                      @method('DELETE')
